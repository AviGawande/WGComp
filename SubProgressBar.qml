...
import QtQuick 2.15
import QtQuick.Controls 2.15 // Needed for Timer if not already imported in your project
...
Item {
    ...
    property var subStepsCompleted: [false, false, false, false, false]
    property int maxSubSteps: 4 // Dynamic based on current main step
    property bool isAutoRunning: false // Controls auto-progression and visual states

    // REMOVED: subStepClicked signal - no longer user-clickable
    ...
    function getLineState(lineIndex) {
        ...
        var currentStepCompleted = subStepsCompleted[lineIndex];
        var nextStepCompleted = subStepsCompleted[lineIndex + 1];
                        ...
                        Row {
                            ...
                            Repeater {
                                ...
                                delegate: Rectangle {
                                    ...
                                    // Animated dashes when auto-running and this is the active line
                                    SequentialAnimation on opacity {
                                        running: root.isAutoRunning &&
                                            (root.getNextIncompleteStep() === (parent.parent.parent.model.index + 1))
                                        loops: Animation.Infinite
                                        NumberAnimation {
                                            to: 0.3
                                            duration: 300 + (model.index * 100) // Staggered animation
            ...
            Row {
                ...
                Repeater {
                    ...
                    delegate: Rectangle {
                        ...
                        // Hover effect (only if not auto-running)
                        scale: (mouseArea.containsMouse && !isAutoRunning) ? 1.1 : 1.0
                        Behavior on scale {
                            NumberAnimation { duration: 150 }
                        }

                        // REMOVED: MouseArea directly clicking sub-steps.
                        // We will keep a MouseArea to handle the hover effect only
                        MouseArea {
                            id: mouseArea
                            anchors.fill: parent
                            hoverEnabled: !isAutoRunning // Only enable hover if not auto-running
